...
export function getSingleItemFromAPI(idParams) {
  return new Promise( (resolve) => {
    setTimeout( () => {
      let movieRequested = moviesDB.find( movie => movie.id === Number(idParams))
      resolve(movieRequested)
    }, 2000 )
  });
}

export function getItemFromAPIbyCategory(category) {
  return new Promise( (resolve) => {
    setTimeout( () => {
      let movieCategoryRequested = moviesDB.filter( movie => movie.movie_gender === category)
      resolve(movieCategoryRequested)
    }, 1000 )
  });
}
